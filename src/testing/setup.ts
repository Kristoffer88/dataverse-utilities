...
function validateSetupOptions(options: DataverseSetupOptions): void {
  ...
  if (tokenRefreshInterval !== undefined) {
    if (
      typeof tokenRefreshInterval !== 'number' ||
      tokenRefreshInterval < 60000 ||
      tokenRefreshInterval > 3600000
    ) {
      throw new Error(
        'tokenRefreshInterval must be a number between 60000ms (1 minute) and 3600000ms (1 hour)'
      )
    }
  }
  ...
  return vi.fn(async (input: string | URL | Request, init?: RequestInit) => {
    try {
      ...
      if (typeof originalFetch === 'function' && getToken() && !getToken()?.startsWith('mock-')) {
        return originalFetch(fullUrl, finalInit)
      }
...
export async function setupDataverse(options: DataverseSetupOptions): Promise<void> {
  try {
    ...
    if (enableConsoleLogging) {
      ...
      console.log(
        '🔒 Security features enabled: URL validation, token sanitization, development-only enforcement'
      )
    }
  } catch (error) {
