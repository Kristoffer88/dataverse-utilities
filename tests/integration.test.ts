...
describe.skipIf(!process.env.RUN_INTEGRATION_TESTS)('Dataverse Integration Tests', () => {
  ...
  it('should fetch real data from dataverse API', async () => {
    ...
    if (response.ok) {
      ...
      console.log(`⚠️  Integration test failed with status ${response.status} - likely no Azure CLI token`);
    }
  });

  it('should handle OData queries correctly', async () => {
    const response = await fetch('/api/data/v9.1/pum_initiatives?$select=pum_name&$filter=statecode eq 0&$top=3');
